...
public class ChestKoolawBoatEntityRenderer extends EntityRenderer<ChestKoolawBoatEntity> {

    private final ChestKoolawBoatEntityModel model;
    private final Identifier texture;


    public ChestKoolawBoatEntityRenderer(EntityRendererFactory.Context ctx) {
        super(ctx);
        this.shadowRadius = 0.8F;
        this.model = new ChestKoolawBoatEntityModel(ctx.getPart(LirothClient.MODEL_CHEST_KOOLAW_BOAT_LAYER), true);
        this.texture = new Identifier(Liroth.MOD_ID, "textures/entity/chest_boat/koolaw.png");
    }

    public void render(ChestKoolawBoatEntity boatEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        float k;
        matrixStack.push();
        ...
        ChestKoolawBoatEntityModel model = this.model;
        Identifier identifier = texture;
        ChestKoolawBoatEntityModel boatEntityModel = model;
    ...
    @Override
    public Identifier getTexture(ChestKoolawBoatEntity entity) {
        return this.texture;
    }
