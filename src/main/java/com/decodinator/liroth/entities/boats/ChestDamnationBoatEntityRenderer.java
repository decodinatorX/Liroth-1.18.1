...
public class ChestDamnationBoatEntityRenderer extends EntityRenderer<ChestDamnationBoatEntity> {

    private final ChestDamnationBoatEntityModel model;
    private final Identifier texture;


    public ChestDamnationBoatEntityRenderer(EntityRendererFactory.Context ctx) {
        super(ctx);
        this.shadowRadius = 0.8F;
        this.model = new ChestDamnationBoatEntityModel(ctx.getPart(LirothClient.MODEL_CHEST_DAMNATION_BOAT_LAYER), true);
        this.texture = new Identifier(Liroth.MOD_ID, "textures/entity/chest_boat/damnation.png");
    }

    public void render(ChestDamnationBoatEntity boatEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
        float k;
        matrixStack.push();
        ...
        ChestDamnationBoatEntityModel model = this.model;
        Identifier identifier = texture;
        ChestDamnationBoatEntityModel boatEntityModel = model;
    ...
    @Override
    public Identifier getTexture(ChestDamnationBoatEntity entity) {
        return this.texture;
    }
