...
public class KoolawBoatEntity extends Entity {
    ...
    private KoolawBoatEntity.Location location;
    private KoolawBoatEntity.Location lastLocation;
    ...
    public KoolawBoatEntity(EntityType<? extends KoolawBoatEntity> type, World world) {
        super(type, world);
        this.paddlePhases = new float[2];
//        this.inanimate = true;
    }

    public KoolawBoatEntity(World world, double x, double y, double z) {
        this(LirothEntities.KOOLAW_BOAT, world);
    ...
    public void tick() {
        ...
        if (this.location != KoolawBoatEntity.Location.UNDER_WATER && this.location != KoolawBoatEntity.Location.UNDER_FLOWING_WATER) {
    ...
    private KoolawBoatEntity.Location checkLocation() {
        KoolawBoatEntity.Location location = this.getUnderWaterLocation();
        if (location != null) {
            this.waterLevel = this.getBoundingBox().maxY;
            return location;
        } else if (this.checkBoatInWater()) {
            return KoolawBoatEntity.Location.IN_WATER;
        } else {
            float f = this.method_7548();
            if (f > 0.0F) {
                this.field_7714 = f;
                return KoolawBoatEntity.Location.ON_LAND;
            } else {
                return KoolawBoatEntity.Location.IN_AIR;
            }
        }
    }
    ...
    @Nullable
    private KoolawBoatEntity.Location getUnderWaterLocation() {
        Box box = this.getBoundingBox();
                ...
                for(int q = m; q < n; ++q) {
                    ...
                    if (fluidState.isIn(FluidTags.WATER) && d < (double)((float)mutable.getY() + fluidState.getHeight(this.world, mutable))) {
                        if (!fluidState.isStill()) {
                            return KoolawBoatEntity.Location.UNDER_FLOWING_WATER;
                        }
        ...
        return bl ? KoolawBoatEntity.Location.UNDER_WATER : null;
    }
    ...
    private void updateVelocity() {
        ...
        if (this.lastLocation == KoolawBoatEntity.Location.IN_AIR && this.location != KoolawBoatEntity.Location.IN_AIR && this.location != KoolawBoatEntity.Location.ON_LAND) {
            ...
            this.location = KoolawBoatEntity.Location.IN_WATER;
        } else {
            if (this.location == KoolawBoatEntity.Location.IN_WATER) {
                f = (this.waterLevel - this.getY()) / (double)this.getHeight();
                this.velocityDecay = 0.9F;
            } else if (this.location == KoolawBoatEntity.Location.UNDER_FLOWING_WATER) {
                e = -7.0E-4D;
                this.velocityDecay = 0.9F;
            } else if (this.location == KoolawBoatEntity.Location.UNDER_WATER) {
                f = 0.009999999776482582D;
                this.velocityDecay = 0.45F;
            } else if (this.location == KoolawBoatEntity.Location.IN_AIR) {
                this.velocityDecay = 0.9F;
            } else if (this.location == KoolawBoatEntity.Location.ON_LAND) {
        ...
        if (!this.hasVehicle()) {
            if (onGround) {
                if (this.fallDistance > 3.0F) {
                    if (this.location != KoolawBoatEntity.Location.ON_LAND) {
    ...
    public boolean isSubmergedInWater() {
        return this.location == KoolawBoatEntity.Location.UNDER_WATER || this.location == KoolawBoatEntity.Location.UNDER_FLOWING_WATER;
    }
