...
public class PetrifiedDamnationBoatItem extends Item {

    private static final Predicate<Entity> RIDERS;

    public PetrifiedDamnationBoatItem(Settings settings) {
        super(settings);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (hitResult.getType() == net.minecraft.util.hit.HitResult.Type.MISS) {
            ...
            if (hitResult.getType() == net.minecraft.util.hit.HitResult.Type.BLOCK) {
            	PetrifiedDamnationBoatEntity vec3d2 = new PetrifiedDamnationBoatEntity(world, hitResult.getPos().x, hitResult.getPos().y, hitResult.getPos().z);
