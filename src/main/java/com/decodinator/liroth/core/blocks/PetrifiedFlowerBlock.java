...
public class PetrifiedFlowerBlock
extends Block {
    public static final int MAX_AGE = 5;
    public static final IntProperty AGE = Properties.AGE_5;
    private final PetrifiedPlantBlock plantBlock;

    protected PetrifiedFlowerBlock(PetrifiedPlantBlock plantBlock, AbstractBlock.Settings settings) {
        super(settings);
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (blockState.isOf(Blocks.END_STONE)) {
            ...
            for (int k = 0; k < 4; ++k) {
                ...
                if (!blockState2.isOf(LirothBlocks.SOULLESS_SOIL)) break;
                bl2 = true;
                break;
            }
        ...
        if (bl && PetrifiedFlowerBlock.isSurroundedByAir(world, blockPos, null) && world.isAir(pos.up(2))) {
            ...
            for (int l = 0; l < j; ++l) {
                ...
                if (!world.isAir(blockPos2) || !world.isAir(blockPos2.down()) || !PetrifiedFlowerBlock.isSurroundedByAir(world, blockPos2, direction.getOpposite())) continue;
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        BlockState blockState = world.getBlockState(pos.down());
        if (blockState.isOf(this.plantBlock) || blockState.isOf(LirothBlocks.SOULLESS_SOIL)) {
            return true;
        }
        if (!blockState.isAir()) {
            return false;
    ...
    public static void generate(WorldAccess world, BlockPos pos, Random random, int size) {
        world.setBlockState(pos, ((PetrifiedPlantBlock)LirothBlocks.PETRIFIED_PLANT).withConnectionProperties(world, pos), Block.NOTIFY_LISTENERS);
        PetrifiedFlowerBlock.generate(world, pos, random, pos, size, 0);
    }

    private static void generate(WorldAccess world, BlockPos pos, Random random, BlockPos rootPos, int size, int layer) {
        PetrifiedPlantBlock chorusPlantBlock = (PetrifiedPlantBlock)LirothBlocks.PETRIFIED_PLANT;
        ...
        for (int j = 0; j < i; ++j) {
            ...
            if (!PetrifiedFlowerBlock.isSurroundedByAir(world, blockPos, null)) {
                return;
            }
        ...
        if (layer < 4) {
            ...
            for (int l = 0; l < k; ++l) {
                ...
                if (Math.abs(blockPos2.getX() - rootPos.getX()) >= size || Math.abs(blockPos2.getZ() - rootPos.getZ()) >= size || !world.isAir(blockPos2) || !world.isAir(blockPos2.down()) || !PetrifiedFlowerBlock.isSurroundedByAir(world, blockPos2, direction.getOpposite())) continue;
                bl = true;
                ...
                PetrifiedFlowerBlock.generate(world, blockPos2, random, rootPos, size, layer + 1);
            }
        }
        if (!bl) {
            world.setBlockState(pos.up(i), (BlockState)LirothBlocks.PETRIFIED_FLOWER.getDefaultState().with(AGE, 5), Block.NOTIFY_LISTENERS);
        }
    }
