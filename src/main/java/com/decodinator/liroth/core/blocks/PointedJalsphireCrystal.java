...
Waterloggable {
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        Direction direction2 = PointedJalsphireCrystal.getDirectionToPlaceAt(worldAccess, blockPos = ctx.getBlockPos(), direction = ctx.getVerticalPlayerLookDirection().getOpposite());
        if (direction2 == null) {
            return null;
        }
    ...
    public static void tryGrow(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (!PointedJalsphireCrystal.canGrow(blockState, blockState2 = world.getBlockState(pos.up(2)))) {
            return;
        }
