...
public class LirothianPetroleumCampfireBlock 
...
implements Waterloggable {
    ...
    public LirothianPetroleumCampfireBlock(boolean emitsParticles, int fireDamage, AbstractBlock.Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ItemStack itemStack;
        LirothianPetroleumCampfireBlockEntity LirothianPetroleumCampfireBlockEntity;
        Optional<CampfireCookingRecipe> optional;
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (blockEntity instanceof LirothianPetroleumCampfireBlockEntity && (optional = (LirothianPetroleumCampfireBlockEntity = (LirothianPetroleumCampfireBlockEntity)blockEntity).getRecipeFor(itemStack = player.getStackInHand(hand))).isPresent()) {
            if (!world.isClient && LirothianPetroleumCampfireBlockEntity.addItem(player, player.getAbilities().creativeMode ? itemStack.copy() : itemStack, optional.get().getCookTime())) {
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        ...
        BlockEntity blockEntity = world.getBlockEntity(pos);
        if (blockEntity instanceof LirothianPetroleumCampfireBlockEntity) {
            ItemScatterer.spawn(world, pos, ((LirothianPetroleumCampfireBlockEntity)blockEntity).getItemsBeingCooked());
        }
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighborState, WorldAccess world, BlockPos pos, BlockPos neighborPos) {
        ...
        return super.getStateForNeighborUpdate(neighborState, direction, neighborState, world, pos, neighborPos);
    }
    ...
    public static void extinguish(@Nullable Entity entity, WorldAccess world, BlockPos pos, BlockState state) {
        ...
        if (world.isClient()) {
            for (int i = 0; i < 20; ++i) {
                LirothianPetroleumCampfireBlock.spawnSmokeParticle((World)world, pos, state.get(SIGNAL_FIRE), true);
            }
        }
        if ((blockEntity = world.getBlockEntity(pos)) instanceof LirothianPetroleumCampfireBlockEntity) {
            ((LirothianPetroleumCampfireBlockEntity)blockEntity).spawnItemsBeingCooked();
        }
    ...
    public boolean tryFillWithFluid(WorldAccess world, BlockPos pos, BlockState state, FluidState fluidState) {
        if (!state.get(Properties.WATERLOGGED).booleanValue() && fluidState.getFluid() == Fluids.WATER) {
            ...
            if (bl) {
                ...
                LirothianPetroleumCampfireBlock.extinguish(null, world, pos, state);
            }
    ...
    public static boolean isLitCampfireInRange(World world, BlockPos pos) {
        for (int i = 1; i <= 5; ++i) {
            ...
            if (LirothianPetroleumCampfireBlock.isLitCampfire(blockState)) {
                return true;
            }
            ...
            return LirothianPetroleumCampfireBlock.isLitCampfire(blockState2);
        }
        return false;
    }
    ...
    public BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new LirothianPetroleumCampfireBlockEntity(pos, state);
    }
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        if (world.isClient) {
            if (state.get(LIT).booleanValue()) {
                return LirothianPetroleumCampfireBlock.checkType(type, Liroth.LIROTHIAN_PETROLEUM_CAMPFIRE_BLOCK_ENTITY, LirothianPetroleumCampfireBlockEntity::clientTick);
            }
        } else {
            if (state.get(LIT).booleanValue()) {
                return LirothianPetroleumCampfireBlock.checkType(type, Liroth.LIROTHIAN_PETROLEUM_CAMPFIRE_BLOCK_ENTITY, LirothianPetroleumCampfireBlockEntity::litServerTick);
            }
            return LirothianPetroleumCampfireBlock.checkType(type, Liroth.LIROTHIAN_PETROLEUM_CAMPFIRE_BLOCK_ENTITY, LirothianPetroleumCampfireBlockEntity::unlitServerTick);
        }
        return null;
    }
