...
extends Feature<DripstoneClusterFeatureConfig> {
    public PetrifiedCrystalClusterFeature(Codec<DripstoneClusterFeatureConfig> codec) {
        super(codec);
    }
    ...
    private void generate(StructureWorldAccess world, Random random, BlockPos pos, int localX, int localZ, float wetness, double dripstoneChance, int height, float density, DripstoneClusterFeatureConfig config) {
        ...
        boolean bl;
        Optional<CaveSurface> optional = CaveSurface.create(world, pos, config.floorToCeilingSearchRange, PetrifiedCrystalHelper::canGenerate, PetrifiedCrystalHelper::canReplaceOrLava);
        ...
        if (optionalInt3.isPresent()) {
        	PetrifiedCrystalHelper.generatePointedDripstone(world, pos.withY(optionalInt3.getAsInt() + 1), Direction.UP, t, bl4);
        }
    }
    ...
    private boolean canWaterSpawn(StructureWorldAccess world, BlockPos pos) {
        ...
        if (blockState.isOf(Blocks.WATER) || blockState.isOf(LirothBlocks.PETRIFIED_CRYSTAL_BLOCK) || blockState.isOf(LirothBlocks.POINTED_PETRIFIED_CRYSTAL)) {
            return false;
        }
