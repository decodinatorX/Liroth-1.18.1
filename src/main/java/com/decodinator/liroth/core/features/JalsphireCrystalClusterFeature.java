...
public class JalsphireCrystalClusterFeature
extends Feature<DripstoneClusterFeatureConfig> {
    public JalsphireCrystalClusterFeature(Codec<DripstoneClusterFeatureConfig> codec) {
        super(codec);
    }
    ...
    public boolean generate(FeatureContext<DripstoneClusterFeatureConfig> context) {
        ...
        if (!JalsphireCrystalHelper.canGenerate(structureWorldAccess, blockPos)) {
            return false;
        }
    ...
    private void generate(StructureWorldAccess world, Random random, BlockPos pos, int localX, int localZ, float wetness, double dripstoneChance, int height, float density, DripstoneClusterFeatureConfig config) {
        ...
        boolean bl;
        Optional<CaveSurface> optional = CaveSurface.create(world, pos, config.floorToCeilingSearchRange, JalsphireCrystalHelper::canGenerate, JalsphireCrystalHelper::canReplaceOrLava);
        ...
        if (optionalInt.isPresent()) {
        	JalsphireCrystalHelper.generatePointedDripstone(world, pos.withY(optionalInt.getAsInt() - 1), Direction.DOWN, m, bl4);
        }
        if (optionalInt3.isPresent()) {
        	JalsphireCrystalHelper.generatePointedDripstone(world, pos.withY(optionalInt3.getAsInt() + 1), Direction.UP, t, bl4);
        }
    }
    ...
    private boolean canWaterSpawn(StructureWorldAccess world, BlockPos pos) {
        ...
        if (blockState.isOf(Blocks.WATER) || blockState.isOf(LirothBlocks.JALSPHIRE_CRYSTAL_BLOCK) || blockState.isOf(LirothBlocks.POINTED_JALSPHIRE_CRYSTAL)) {
            return false;
        }
    ...
    private void placeDripstoneBlocks(StructureWorldAccess world, BlockPos pos, int height, Direction direction) {
        ...
        for (int i = 0; i < height; ++i) {
            if (!JalsphireCrystalHelper.generateDripstoneBlock(world, mutable)) {
                return;
            }
