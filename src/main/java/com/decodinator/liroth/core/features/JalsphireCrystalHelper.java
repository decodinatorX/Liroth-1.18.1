...
public class JalsphireCrystalHelper {
    ...
    protected static boolean canGenerate(WorldAccess world, BlockPos pos) {
        return world.testBlockState(pos, JalsphireCrystalHelper::canGenerate);
    }

    protected static boolean canGenerateOrLava(WorldAccess world, BlockPos pos) {
        return world.testBlockState(pos, JalsphireCrystalHelper::canGenerateOrLava);
    }

    protected static void getDripstoneThickness(Direction direction, int height, boolean merge, Consumer<BlockState> callback) {
        if (height >= 3) {
            callback.accept(JalsphireCrystalHelper.getState(direction, Thickness.BASE));
            for (int i = 0; i < height - 3; ++i) {
                callback.accept(JalsphireCrystalHelper.getState(direction, Thickness.MIDDLE));
            }
        }
        if (height >= 2) {
            callback.accept(JalsphireCrystalHelper.getState(direction, Thickness.FRUSTUM));
        }
        if (height >= 1) {
            callback.accept(JalsphireCrystalHelper.getState(direction, merge ? Thickness.TIP_MERGE : Thickness.TIP));
        }
    }
    ...
    protected static void generatePointedDripstone(WorldAccess world, BlockPos pos, Direction direction, int height, boolean merge) {
        if (!JalsphireCrystalHelper.canReplace(world.getBlockState(pos.offset(direction.getOpposite())))) {
            return;
        }
        ...
        JalsphireCrystalHelper.getDripstoneThickness(direction, height, merge, state -> {
            if (state.isOf(LirothBlocks.POINTED_JALSPHIRE_CRYSTAL)) {
                state = (BlockState)state.with(PointedJalsphireCrystal.WATERLOGGED, world.isWater(mutable));
            }
    ...
    protected static boolean generateDripstoneBlock(WorldAccess world, BlockPos pos) {
        ...
        if (blockState.isIn(BlockTags.DRIPSTONE_REPLACEABLE_BLOCKS)) {
            world.setBlockState(pos, LirothBlocks.JALSPHIRE_CRYSTAL_BLOCK.getDefaultState(), Block.NOTIFY_LISTENERS);
            return true;
        }
        return false;
    }
    ...
    private static BlockState getState(Direction direction, Thickness thickness) {
        return (BlockState)((BlockState)LirothBlocks.POINTED_JALSPHIRE_CRYSTAL.getDefaultState().with(PointedJalsphireCrystal.VERTICAL_DIRECTION, direction)).with(PointedJalsphireCrystal.THICKNESS, thickness);
    }

    public static boolean canReplaceOrLava(BlockState state) {
        return JalsphireCrystalHelper.canReplace(state) || state.isOf(Blocks.LAVA);
    }

    public static boolean canReplace(BlockState state) {
        return state.isOf(LirothBlocks.JALSPHIRE_CRYSTAL_BLOCK) || state.isIn(BlockTags.DRIPSTONE_REPLACEABLE_BLOCKS);
    }
